...
```
`console.log` prints something to the terminal, while `return` is used to define the expected output of a function.
```

**_What's the difference between defining and calling a function?_**

```
You define a function by creating it, then you can call the function by passing arguments into it after it's been defined.
```
...
```
Any combination of characters, enclosed in either single or double quotation marks (or backticks, if using string concatentation).
```

**_Number_**

```
A number that the program can perform mathematical operations on.
```
...
```
A collection of properties assigned to a variable, enclosed in brackets and accessible by their indices.
```
